...
union_refs = {
    arm["$ref"].rsplit("/", 1)[-1]
    for arm in schema["$defs"]["PublicValidationContract"]["anyOf"]
}
assert union_refs == PUBLIC_MODELS
...
assert "union PublicValidationContract" in typespec, "missing TypeSpec public contract union"
